...
export default function BankStatementReconciliationModal({ open, onOpenChange }) {
  ...
  const [hideProcessed, setHideProcessed] = useState(false); // TOGGLE
  ...
  const rows = useMemo(() => {
      ...
      return statementRows.map(row => {
          ...
          const processed = reconciledTxs.find(t => toCents(t.amount) === toCents(row.amount) && Math.abs(differenceInCalendarDays(parseISO(t.date), parseISO(row.date))) <= 4);
  ...
  const filteredRows = useMemo(() => hideProcessed ? rows.filter(r => r.status !== 'processed') : rows, [rows, hideProcessed]);
  ...
  const exec = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries(); toast.success('Conciliação finalizada!'); setShowSummary(false); onOpenChange(false); }
  });

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-7xl h-[90vh] flex flex-col">
        <DialogHeader><DialogTitle>Conciliação de Extrato</DialogTitle></DialogHeader>
            ...
            <Table>
                ...
                <TableBody>
                    {filteredRows.map(row => (
                        <TableRow key={row.id} className={row.status === 'processed' ? 'opacity-50 bg-slate-50' : ''}>
